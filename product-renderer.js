...
class ProductRenderer {
    ...
    slugify(text) {
        ...
        return text
            ...
            .replace(/[^\w-]+/g, '')             // Remove all remaining non-word chars
    ...
    generateFeaturedSection() {
        ...
        return `
            <section class="featured-dominance" id="editors-pick-featured">
    ...
    handleHashScroll() {
        ...
        const performScroll = () => {
            ...
            } else if (hash === '#editors-pick-featured' || hash === '#featured') {
                target = document.querySelector('.featured-dominance');
            } else {
                // Remove the # and use getElementById for direct ID matching
                const id = hash.substring(1);
                target = document.getElementById(id);

                // Fallback for older IDs or custom ones if any
                if (!target) target = document.querySelector(hash);
            }

            if (target) {
                // Add a small delay for mobile browsers and to ensure layout stability
                setTimeout(() => {
                    ...
                    // If it's a "details-" hash, auto-open the toggle
                    if (hash.includes('details-')) {
                        toggleDetails(hash.substring(1));
                    }
                }, 100);
            }
        };
 ...
 */
function toggleDetails(id, buttonElement = null) {
    ...
    const btn = buttonElement || document.querySelector(`a[href="#${id}"]`);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Handle Details Toggle Clicks (Direct Listener for responsiveness)
    document.addEventListener('click', (e) => {
        const toggleBtn = e.target.closest('.btn-details-toggle');
        if (toggleBtn) {
            e.preventDefault();
            const href = toggleBtn.getAttribute('href');
            if (href && href.startsWith('#')) {
                toggleDetails(href.substring(1), toggleBtn);
            }
        }
    });

    window.addEventListener('hashchange', () => {
        // Fallback for direct hash navigation, but click listener handles immediate interaction
        if (window.productRenderer) window.productRenderer.handleHashScroll();
    });

    document.querySelectorAll('.hero-ctas a').forEach(btn => {
        btn.addEventListener('click', (e) => {
            // Delay slightly to ensure render is complete or scroll target is stable
